...
and the loop trajectory in `.cairndev/loop.md` as well as the design contract.
...
3. Read `.cairndev/loop.md` when a loop trajectory is configured.
4. Identify affected modules and boundaries.
5. Produce a concise implementation plan.
6. Name the smallest viable change.
7. Implement with tests.
8. Run the project’s test/lint commands.
9. Run `cairndev check .` if available.
10. Run configured language formatters/linters/type checkers when available.
11. Update `.cairndev/goal.yaml` and `.cairndev/loop.md` after verified multi-round work.
12. Summarize design impact and any contract violations.
